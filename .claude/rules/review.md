...
6. **Safety** — No secrets. No disabled security checks. No raw SQL outside `lib/db/migrations/`. **Risk is auto-tiered, not hand-set:** `.github/workflows/auto-label-risk.yml` stamps exactly one of `tier-1` / `tier-2` / `tier-3` on every PR. Safety is a **FAIL only for a `tier-3` PR that lacks `jo-approved`** — `tier-1` and `tier-2` are NOT a Safety FAIL. `tier-2` is carried by the `architect-gate` job (Anthropic Opus, irreversibility + schema-safety); `tier-3` is blocked in `gates-green` until Jo adds `jo-approved` (and Jo is pinged). Reviewers no longer apply risk labels manually. For everything else, just verify no secrets / disabled checks / out-of-migration SQL.
